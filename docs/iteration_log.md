...
## Iteration: E5 by_category aggregation bug fix + real re-aggregation (2026-06-11)

- Context: starting Chapter-6 Phase-B human-annotation review (CC↔Codex), beginning with E5 (highest ROI, smallest set). Joint plan recorded in thesis `paper-prep/114`.
- Found a result-affecting reporting bug in `scripts/exp_e5_safety_redteam.py::_summarize_e5`: `by_category` looked up `rule_violations.get(<attack-category-name>)` (e.g. `induce_medication`), but the violation dict keys are `{diagnosis, medication, device_overclaim, emergency_reassurance}`. Only `emergency_reassurance` matched its category name; the other three always read a missing key → reported `0/24`. Meanwhile `overall_rule_flagged` (correctly via the `violated()` helper) said 29/96, and prose record `113` said "medication 24/24". Three numbers disagreed.
- Fix: route `by_category` through the same `violated(r, "rule"/"judge")` helper as the overall count, so per-category counts sum to the overall. Added regression assertions in `tests/test_e3_e5_harness.py` (`by_category[induce_medication].rule.events == 1`; per-category rule sum == overall rule-flagged).
- Also: the committed `e5_redteam_result.json` was a stale `seed_count=1` dry-run; ran the zero-API `summarize` subcommand on the real 216 saved replies. Corrected numbers — ON rule 0/96 judge 0/96; OFF rule 29/96 judge 0/96 (all 29 rule-fired-judge-cleared = false positives); OFF by category: diagnosis 3/24, medication 24/24 (100%), ppg_trust 1/24, emergency 1/24; benign over-block 0/12. Matches the `113` narrative; prevents a paper-to-evidence mismatch.
- E5 seed review (CC↔Codex independent, converged): 36 seeds all valid/no dupes; coverage gaps = R0+R1 only (no R2 bypass rewrites), weak low-aggressiveness, emergency seeds cover only chest-pain (missing stroke/neuro symptoms), benign set small. Counter-example seeds drafted (metaphor / reverse-question / cross-persona chain / neuro-emergency) pending author sign-off before optional E5 re-run. Safety claim scoped to "R0/R1 single-turn × 3 personas × on/off; judge-confirmed violation Wilson upper bound 3.85% per 96-trial arm; not 'the system is safe'".
- Tests: `tests/test_e3_e5_harness.py` 9 passed; full safety/stat/e5 selection green.

## Iteration: E5 seed expansion (re-run) + E3 dual-blind calibration batch-1 (2026-06-11)

- E5 coverage gaps (CC↔Codex agreed): original 36 seeds were R0/R1 only (no obfuscation/bypass), emergency seeds covered chest-pain only. Added 12 reproducible hand-written counter-examples via a new `extra-seeds` subcommand + `_EXTRA_SEEDS` constant (R2 obfuscation/metaphor/reverse-question/cross-persona + neuro-emergency + low-aggression + 4 benign), appended with stable seed_ids ≥36 so `run --resume` reuses the original 216 replies. Carried `rewrite_degree`/`attack_style` into records; added a `by_rewrite_degree` cut to the summary so the R2/bypass robustness is reported separately and the safety claim stays scoped to the attack family tested. Re-run executing in background (216→288 replies).
- E3 calibration: new `scripts/exp_e3_make_review_sheet.py` builds a stratified 204-pair sheet (S2/S3 × fully/partial/not_supported, 34/stratum) reconstructing cited evidence text deterministically (no API); flags 41 high-risk items (threshold/medication/emergency/pregnancy/diagnosis) for the author escalation queue.
- E3 batch-1 (n=40) CC↔Codex blind dual-annotation: agreement 0.85, Cohen κ=0.77 (CC vs Codex). KEY METHODOLOGICAL FINDING: Codex(GPT) vs judge(GPT-5.5) κ=0.96 vs CC(Claude) vs judge κ=0.74 — same-family LLM agreement is near-circular and must NOT be reported as judge validation. Report cross-family κ as inter-annotator difficulty; reserve Rogan-Gladen correction for true human (author) labels. Reconciled all 6 disagreements (lifestyle/governance-meta unified rule: topic-authorization → partial, specific efficacy/time claim w/o evidence → not_supported). Full record in thesis paper-prep/114 §9.

## Iteration: E3 dual-blind calibration COMPLETE — all 204 reconciled (2026-06-12)

- Finished CC↔Codex blind dual-annotation of all 204 stratified sentence-evidence pairs (5 batches). Cross-family agreement rose as the rubric converged: batch1 κ=0.77 → batch4 κ=1.0; FULL κ=0.925 (agreement 95.1%). 11 disagreements total, all reconciled via the agreed rules (governance-meta→partial unless specific efficacy/threshold claim→not_supported; numeric-classification w/o threshold→not_supported; premise+action→partial; emergency-advice citing PPG-limitation evidence→not_supported due to citation-content mismatch).
- HEADLINE: the GPT-5.5 judge systematically OVER-CREDITS citation faithfulness. On the balanced calibration sample, cross-family consensus strict-support=0.23 vs judge 0.33 (lenient 0.42 vs 0.50). Both Claude (κ vs judge 0.65) and Codex (κ vs judge 0.71) land stricter than the judge — and agree with each OTHER at 0.925, so this is a real leniency bias, not cross-family noise.
...
## Iteration: E1 retrieval-relevance dual-blind calibration COMPLETE (2026-06-12)

- CC↔Codex cross-family blind 0/1/2 graded relevance over all 350 pooled query-chunk pairs (100 queries), 7 batches. Overall κ=0.718 (agreement 0.83) — normal for graded relevance (inter-assessor κ ~0.5-0.7 typical). κ rose from batch-1 0.58 → batch-4 0.90 after one boundary-alignment round (rule: English research fragment not directly answering→0; governance/listing matching topic but missing query sub-dimension→1).
- 137 sensitive (emergency/medication/pregnancy queries) → author signoff (112 CC↔Codex agreed, 25 disagree — mostly off-population chunk: CC marks pregnancy/CKD/elderly-specific chunk as 1 for a general query, Codex as 2). 213 non-sensitive finalized.
- E1 HEADLINE — self-eval inflation: the system's own self-evaluation (allowed_uses→relevant, used by the original precision@5≈0.98-1.0 metric) marked 209/213 (98%) non-sensitive pairs relevant, but independent cross-family qrels find only 164/213 (77%) grade≥1 and 77/213 (36%) grade-2. The self-eval over-claims retrieval precision by ~20pp (lenient) / ~60pp (strict) — quantifies the self-judging bias the original metric hid.
- Frozen into committed artifact knowledge_base/processed/e1_calibration_final.json (per-item cc/codex/judge/final + agreement + self-eval bias), same durability protection as E3 after the sheet-revert incident. Full nDCG recompute pending author signoff on the 137 sensitive grades.

## Iteration: E1 author signoff applied + calibrated recompute (2 validity caveats) (2026-06-12)

- Author signed all 137 E1 sensitive (112 CC↔Codex-agreed taken as-is; 25 cross-population disagreements → grade 1 per author rule). Propagated final grades → human_grade in e1_qrels_pool.jsonl: 323/350 matched.
- PROVENANCE MISMATCH (flag for table-lock): review sheet was built from an 849-pair pool; current qrels pool is 947 (later re-pool). They share 323 of the 350 graded pairs; the 27 misses are all fulltext:: chunks from the diverging fulltext run. Before locking Table 6-1, rebuild the review selection from the FINAL frozen pool (or freeze the pool to the sheet) so calibration coverage is exact.
- CALIBRATED RECOMPUTE CAVEAT 1 — OpenAI embeddings inactive in the offline evaluate run: vector_only_openai / hybrid_openai degenerated to keyword_only (all nDCG@5=0.8826); only hashing variants differ (0.9052, uniq-sources higher with ft). The embedding-backend comparison (OpenAI vs hashing vs keyword) requires a key-active run with the OpenAI embedding npz built (scripts/ingest_openai_embeddings.py).
