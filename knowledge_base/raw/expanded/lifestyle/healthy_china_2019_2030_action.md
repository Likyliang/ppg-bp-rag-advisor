---
source_id: healthy_china_2019_2030_action
title: 健康中国行动（2019-2030年）
source_type: national_policy_plan_pdf
organization: 健康中国行动推进委员会
region: CN
topic: lifestyle
language: zh
url: https://www.nhc.gov.cn/cms-search/downFiles/470339610aea4a7887d0810b4c00c9bd.pdf
doi: ''
pmid: ''
year: '2019'
last_accessed: '2026-05-27'
evidence_class: official_health_education
source_quality_score: '21'
allowed_uses:
- lifestyle
- home_bp_monitoring
- remeasurement
- disclaimer
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: ef395958e8d6bd25
safety_level: standard
---

## 来源摘要

《健康中国行动（2019-2030年）》提供中国慢病防控、心脑血管疾病防治、健康生活方式和高血压规范管理率目标的政策背景。

## 可用于报告的要点

- 中国高血压防控强调健康生活方式、基层管理和长期规范管理。
- 该来源适合用于公共卫生背景和中国场景解释。
- 政策目标不能被转写为个体诊断或治疗建议。

## 实现使用说明

- 用于中国公共卫生背景、lifestyle 和免责声明；不用于个体化临床决策。

## 全文候选与下载材料摘要

### 健康中国行动（2019-2030年）

- candidate_id: `healthy_china_2019_2030_action_pdf`
- access_mode: `public_pdf`
- fulltext_status: downloadable
- source_url: https://www.nhc.gov.cn/cms-search/downFiles/470339610aea4a7887d0810b4c00c9bd.pdf
- doi: n/a
- pmid: n/a
- access_recorded: 2026-05-27
- local_pdf: not saved

#### 摘要入库要点

- 从国家卫健委公开 PDF 归档；本项目只提交政策背景摘要、citation 和访问记录，不提交全文 PDF。
- 该行动方案支持中国高血压防控中的健康生活方式、基层管理和长期规范管理背景。
- 报告引用该来源只用于公共卫生背景和免责声明，不用于个体化临床判断。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `7ec77a531a078b8b`

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。

