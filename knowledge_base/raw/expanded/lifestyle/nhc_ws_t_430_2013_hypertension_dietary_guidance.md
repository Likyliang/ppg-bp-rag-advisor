---
source_id: nhc_ws_t_430_2013_hypertension_dietary_guidance
title: 高血压患者膳食指导 WS/T 430-2013
source_type: health_industry_standard_pdf
organization: 国家卫生健康委员会
region: CN
topic: lifestyle
language: zh
url: https://www.nhc.gov.cn/wjw/yingyang/201308/cce5017663e6457d98db7be683c36b4c.shtml
doi: ''
pmid: ''
year: '2013'
last_accessed: '2026-05-27'
evidence_class: validation_standard
source_quality_score: '20'
allowed_uses:
- lifestyle
- home_bp_monitoring
- remeasurement
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: 26ae207561462f33
safety_level: standard
---

## 来源摘要

WS/T 430-2013 是国家卫健委发布的高血压患者膳食指导标准，可作为中国场景饮食建议的标准化背景来源。

## 可用于报告的要点

- 高血压饮食解释应围绕限盐、合理膳食和长期生活方式管理。
- 标准来源可用于支撑报告中的通用饮食原则。
- 本系统不得基于该标准生成针对个人病情的治疗或用药建议。

## 实现使用说明

- 用于 lifestyle 和 remeasurement 背景；与 2023 食养指南搭配使用。

## 全文候选与下载材料摘要

### 高血压患者膳食指导 WS/T 430-2013

- candidate_id: `nhc_ws_t_430_2013_hypertension_dietary_guidance_pdf`
- access_mode: `public_pdf`
- fulltext_status: downloadable
- source_url: https://www.nhc.gov.cn/wjw/yingyang/201308/cce5017663e6457d98db7be683c36b4c.shtml
- doi: n/a
- pmid: n/a
- access_recorded: 2026-05-27
- local_pdf: not saved

#### 摘要入库要点

- 从国家卫健委官方标准页面归档公开 PDF；本项目只提交中文摘要、citation 和访问记录，不提交全文 PDF。
- 该标准支持中国场景下限盐、合理膳食和长期生活方式管理的通用解释。
- 报告引用该来源时只输出一般性健康教育和复测提示，不生成个体化治疗或用药建议。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `0e587b93cb2c5bc2`

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。

