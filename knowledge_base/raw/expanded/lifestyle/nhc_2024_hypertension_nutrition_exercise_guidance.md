---
source_id: nhc_2024_hypertension_nutrition_exercise_guidance
title: 高血压营养和运动指导原则（2024年版）
source_type: national_guidance
organization: 国家卫生健康委办公厅
region: CN
topic: lifestyle
language: zh
url: https://app.www.gov.cn/govdata/gov/202407/02/516820/article.html
doi: ''
pmid: ''
year: '2024'
last_accessed: '2026-05-16'
evidence_class: official_health_education
source_quality_score: '25'
allowed_uses:
- lifestyle
- special_population
- home_bp_monitoring
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: c41c422532773f34
safety_level: standard
---

## 来源摘要

国家卫健委 2024 高血压营养和运动指导原则是中文生活方式建议的高优先级本土来源。

## 可用于报告的要点

- 中文报告的减盐、合理膳食、运动和体重管理建议可优先引用该来源。
- 生活方式内容属于健康教育，不承诺个体降压效果。
- 特殊人群仍需医生或营养/运动专业人员个体化评估。

## 实现使用说明

- 用于 CN 场景 lifestyle；不得生成治疗方案或替代医嘱。

## 全文候选与下载材料摘要

### 高血压营养和运动指导原则 2024 PDF

- candidate_id: `nhc_2024_hypertension_nutrition_exercise_pdf`
- access_mode: `public_pdf`
- fulltext_status: local_pdf_available
- source_url: https://app.www.gov.cn/govdata/gov/202407/02/516820/article.html
- doi: n/a
- pmid: n/a
- access_recorded: 2026-05-16
- local_pdf: `nhc_2024_hypertension_nutrition_exercise_guidance.pdf`

#### 摘要入库要点

- 已从国家政务服务平台/政府网站公开 PDF 下载；本项目只提交中文摘要、citation 和访问记录，不提交全文 PDF。
- 该指导原则支持中文报告中的减盐、合理膳食、规律运动、体重管理和生活方式记录等健康教育内容。
- 生活方式建议只能作为一般健康教育，不承诺个体降压效果，也不能替代临床治疗或随访。
- 特殊人群、合并疾病或正在用药用户，应提示咨询医生、营养或运动专业人员后个体化执行。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `e24cca1a5d37e594`

已检测到本地 PDF：nhc_2024_hypertension_nutrition_exercise_guidance.pdf。知识库只使用上述摘要和 citation，不复制全文。

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。

