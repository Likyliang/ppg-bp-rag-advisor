---
source_id: nhc_2023_adult_hypertension_dietary_guideline
title: 成人高血压食养指南（2023年版）
source_type: national_nutrition_guideline_pdf
organization: 国家卫生健康委办公厅
region: CN
topic: lifestyle
language: zh
url: https://www.nhc.gov.cn/sps/c100088/202301/f01895a06c5349ef999f25da833c166d.shtml
doi: ''
pmid: ''
year: '2023'
last_accessed: '2026-05-27'
evidence_class: guideline
source_quality_score: '24'
allowed_uses:
- lifestyle
- home_bp_monitoring
- remeasurement
- special_population
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: 5ffa3d99eaef807c
safety_level: standard
---

## 来源摘要

国家卫健委《成人高血压食养指南（2023年版）》提供中国成人高血压膳食、减钠增钾、体重管理和自我血压监测的官方食养依据。

## 可用于报告的要点

- 高血压管理中的膳食干预应强调减钠增钾、合理膳食、吃动平衡和体重管理。
- 报告可把该来源用于中国用户生活方式解释和血压自我管理提醒。
- 食养指南不能被改写为个体化诊断、处方或替代医生治疗方案。

## 实现使用说明

- 优先用于中国场景 lifestyle、home_bp_monitoring 和 remeasurement；不输出个体化食疗处方。

## 全文候选与下载材料摘要

### 成人高血压食养指南（2023年版）

- candidate_id: `nhc_2023_adult_hypertension_dietary_guideline_pdf`
- access_mode: `public_pdf`
- fulltext_status: local_pdf_available
- source_url: https://www.nhc.gov.cn/sps/c100088/202301/f01895a06c5349ef999f25da833c166d.shtml
- doi: n/a
- pmid: n/a
- access_recorded: 2026-05-27
- local_pdf: `nhc_2023_adult_hypertension_dietary_guideline.pdf`

#### 摘要入库要点

- 从国家卫健委官方公开 PDF 归档；本项目只提交中文摘要、citation 和访问记录，不提交全文 PDF。
- 该指南支持中国成人高血压人群的减钠增钾、合理膳食、吃动平衡、体重管理和血压自我监测解释。
- 报告只能把该来源用于生活方式教育、复测和自我管理提醒；不得生成个体化食疗处方、诊断或治疗建议。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `ea4232c3afda023e`

已检测到本地 PDF：nhc_2023_adult_hypertension_dietary_guideline.pdf。知识库只使用上述摘要和 citation，不复制全文。

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。

