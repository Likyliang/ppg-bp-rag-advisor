---
source_id: bent_2020_wearable_optical_hr_inaccuracy
title: Investigating sources of inaccuracy in wearable optical heart rate sensors
source_type: wearable_optical_sensor_accuracy_study
organization: npj Digital Medicine
region: US
topic: measurement_quality
language: en
url: https://www.nature.com/articles/s41746-020-0226-6
doi: 10.1038/s41746-020-0226-6
pmid: '32047863'
year: '2020'
last_accessed: '2026-05-10'
evidence_class: research_context
source_quality_score: '20'
allowed_uses:
- signal_quality
- research_background
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: a9e2634d36c48e5e
safety_level: standard
---

## 来源摘要

该研究评估可穿戴光学心率传感器误差来源，可作为运动、肤色和设备差异影响光学 PPG 信号的补充证据。

## 可用于报告的要点

- 可穿戴光学传感器误差与设备、活动类型和个体因素有关。
- 运动状态下误差可能高于静息状态。
- 该来源支持光学传感质量提示，不把心率研究外推成血压诊断能力。

## 实现使用说明

- 用于 motion_artifact、skin_tone 和 optical_sensor limitation 背景；不用于血压临床建议。

## 全文候选与下载材料摘要

### Investigating sources of inaccuracy in wearable optical heart rate sensors

- candidate_id: `bent_2020_wearable_optical_hr_inaccuracy_fulltext`
- access_mode: `public_pdf`
- fulltext_status: local_pdf_available
- source_url: https://www.nature.com/articles/s41746-020-0226-6
- doi: 10.1038/s41746-020-0226-6
- pmid: 32047863
- access_recorded: 2026-05-22
- local_pdf: `bent_2020_wearable_optical_hr_inaccuracy.pdf`

#### 摘要入库要点

- 已从公开 Nature/npj Digital Medicine 入口归档 PDF；本项目只提交中文摘要、citation 和访问记录，不提交全文 PDF。
- 该研究支持可穿戴光学传感器误差与设备、活动类型和个体因素有关，运动状态下误差可能增加。
- 报告可用该来源补充运动、肤色/个体因素和设备差异的光学质量背景；不得外推为 PPG 血压诊断能力。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `f980d9de2b261643`

已检测到本地 PDF：bent_2020_wearable_optical_hr_inaccuracy.pdf。知识库只使用上述摘要和 citation，不复制全文。

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。
- 研究或综述来源只用于背景和局限说明，不用于患者级临床建议。

