---
source_id: electronics_2023_ppg_wearable_devices_review
title: Photoplethysmography in Wearable Devices - A Comprehensive Review of Technological
  Advances Current Challenges and Future Directions
source_type: review_pdf
organization: Electronics
region: global
topic: measurement_quality
language: en
url: https://www.mdpi.com/2079-9292/12/13/2923
doi: 10.3390/electronics12132923
pmid: ''
year: '2023'
last_accessed: '2026-05-21'
evidence_class: review
source_quality_score: '23'
allowed_uses:
- signal_quality
- cuffless_ppg_limitations
- research_background
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: 70fc7cb0b0651974
safety_level: standard
---

## 来源摘要

该综述总结可穿戴 PPG 技术进展、挑战和未来方向，可用于说明当前可穿戴 PPG 的运动伪影、传感器限制、算法泛化和真实世界应用边界。

## 可用于报告的要点

- 可穿戴 PPG 仍面临运动、佩戴、传感器、个体差异和算法泛化挑战。
- 报告可用该来源支持 PPG 估算局限和重新采集建议。
- 不能用综述性技术进展替代设备验证或临床指南。

## 实现使用说明

- 用于 signal_quality、cuffless_ppg_limitations 和 research_background。

## 全文候选与下载材料摘要

### Photoplethysmography in Wearable Devices - A Comprehensive Review of Technological Advances Current Challenges and Future Directions

- candidate_id: `electronics_2023_ppg_wearable_devices_review_pdf`
- access_mode: `public_pdf`
- fulltext_status: local_pdf_available
- source_url: https://www.mdpi.com/2079-9292/12/13/2923
- doi: 10.3390/electronics12132923
- pmid: n/a
- access_recorded: 2026-05-21
- local_pdf: `electronics_2023_ppg_wearable_devices_review.pdf`

#### 摘要入库要点

- 已从 MDPI 官方开放 PDF 下载；本项目只提交中文摘要、citation 和访问记录，不提交全文 PDF。
- 该综述支持可穿戴 PPG 仍面临运动、佩戴、传感器、个体差异和算法泛化挑战。
- 报告可用该来源支持 PPG 估算局限和重新采集建议；不得替代设备验证标准或临床指南。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `0f119bad41bcff7e`

已检测到本地 PDF：electronics_2023_ppg_wearable_devices_review.pdf。知识库只使用上述摘要和 citation，不复制全文。

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。
- 研究或综述来源只用于背景和局限说明，不用于患者级临床建议。

