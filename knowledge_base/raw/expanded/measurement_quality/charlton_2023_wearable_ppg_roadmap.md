---
source_id: charlton_2023_wearable_ppg_roadmap
title: The 2023 wearable photoplethysmography roadmap
source_type: roadmap_review
organization: Physiological Measurement
region: global
topic: measurement_quality
language: en
url: https://iopscience.iop.org/article/10.1088/1361-6579/acead2
doi: 10.1088/1361-6579/acead2
pmid: '37494945'
year: '2023'
last_accessed: '2026-05-16'
evidence_class: review
source_quality_score: '24'
allowed_uses:
- signal_quality
- cuffless_ppg_limitations
- device_advice
- research_background
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: a06b8a9f5c3062f2
safety_level: standard
---

## 来源摘要

2023 可穿戴 PPG 路线图提供了运动、肤色、接触、佩戴位置和真实世界可用性等研究挑战的高层框架。

## 可用于报告的要点

- 运动伪影、个体差异、肤色/组织光学特性和佩戴条件是可穿戴 PPG 的关键质量挑战。
- 传感器位置、姿势和接触稳定性会影响光学波形。
- 报告应把这些因素写成采集质量限制，而不是临床判断依据。

## 实现使用说明

- 用于 PPG signal_quality 和 research_background；敏感场景需与 guideline 或 safety_rule 搭配。

## 全文候选与下载材料摘要

### The 2023 wearable photoplethysmography roadmap

- candidate_id: `charlton_2023_wearable_ppg_roadmap_fulltext`
- access_mode: `public_html`
- fulltext_status: local_pdf_available
- source_url: https://iopscience.iop.org/article/10.1088/1361-6579/acead2
- doi: 10.1088/1361-6579/acead2
- pmid: 37494945
- access_recorded: 2026-05-16
- local_pdf: `charlton_2023_wearable_ppg_roadmap.pdf`

#### 摘要入库要点

- 已从 IOP 官方 PDF 链接下载；本项目只提交中文摘要、citation 和访问记录，不提交全文 PDF。
- 可穿戴 PPG 路线图指出，真实世界中的运动、皮肤/组织差异、传感器接触、佩戴位置、算法泛化、标注和验证仍是关键挑战。
- 本系统引用该来源支持 PPG/无袖带局限、运动伪影、肤色/光照、接触状态、传感器位置和研究背景解释。
- 该来源不得被用于推断设备可诊断高血压、可替代袖带血压计或可直接指导治疗。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `014a7cbe1ffb1091`

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。
- 研究或综述来源只用于背景和局限说明，不用于患者级临床建议。

