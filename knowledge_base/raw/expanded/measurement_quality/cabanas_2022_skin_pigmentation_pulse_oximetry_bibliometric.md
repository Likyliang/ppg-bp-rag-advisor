---
source_id: cabanas_2022_skin_pigmentation_pulse_oximetry_bibliometric
title: Skin Pigmentation Influence on Pulse Oximetry Accuracy - A Systematic Review
  and Bibliometric Analysis
source_type: systematic_review_pdf
organization: Sensors
region: global
topic: measurement_quality
language: en
url: https://www.mdpi.com/1424-8220/22/9/3402
doi: 10.3390/s22093402
pmid: ''
year: '2022'
last_accessed: '2026-05-21'
evidence_class: review
source_quality_score: '22'
allowed_uses:
- signal_quality
- research_background
- disclaimer
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: 27bc77c3382a30b5
safety_level: standard
---

## 来源摘要

该系统综述和文献计量分析总结皮肤色素对脉搏氧准确性的影响，可作为光学传感器人群差异和公平性风险的补充背景。

## 可用于报告的要点

- 光学测量可能存在与皮肤色素相关的误差风险。
- 报告可用该来源作为肤色/光学公平性 caveat 的补充证据。
- 该来源不用于 PPG 血压准确性量化或临床建议。

## 实现使用说明

- 用于 signal_quality、research_background 和免责声明。

## 全文候选与下载材料摘要

### Skin Pigmentation Influence on Pulse Oximetry Accuracy - A Systematic Review and Bibliometric Analysis

- candidate_id: `cabanas_2022_skin_pigmentation_pulse_oximetry_bibliometric_pdf`
- access_mode: `public_pdf`
- fulltext_status: local_pdf_available
- source_url: https://www.mdpi.com/1424-8220/22/9/3402
- doi: 10.3390/s22093402
- pmid: n/a
- access_recorded: 2026-05-21
- local_pdf: `cabanas_2022_skin_pigmentation_pulse_oximetry_bibliometric.pdf`

#### 摘要入库要点

- 已从 MDPI/PMC 合法公开入口归档 PDF；本项目只提交中文摘要、citation 和访问记录，不提交全文 PDF。
- 该综述支持光学测量可能存在与皮肤色素相关的误差风险。
- 报告可用该来源作为肤色/光学公平性 caveat 的补充证据；不用于 PPG 血压准确性量化或临床建议。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `f043416055ee3d5e`

已检测到本地 PDF：cabanas_2022_skin_pigmentation_pulse_oximetry_bibliometric.pdf。知识库只使用上述摘要和 citation，不复制全文。

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。
- 研究或综述来源只用于背景和局限说明，不用于患者级临床建议。

