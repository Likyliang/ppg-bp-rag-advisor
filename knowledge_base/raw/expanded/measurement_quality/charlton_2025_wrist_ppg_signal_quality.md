---
source_id: charlton_2025_wrist_ppg_signal_quality
title: Determinants of photoplethysmography signal quality at the wrist
source_type: observational_signal_quality_study
organization: PLOS Digital Health
region: global
topic: measurement_quality
language: en
url: https://journals.plos.org/digitalhealth/article?id=10.1371/journal.pdig.0000585
doi: 10.1371/journal.pdig.0000585
pmid: '40577339'
year: '2025'
last_accessed: '2026-05-16'
evidence_class: research_context
source_quality_score: '23'
allowed_uses:
- signal_quality
- research_background
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: 1d47052e5cb13b7f
safety_level: standard
---

## 来源摘要

该研究分析腕部 PPG 信号质量影响因素，适合支持传感器位置、姿势和个体差异会影响波形质量。

## 可用于报告的要点

- 腕部 PPG 容易受到传感器高度、姿势、位置和个体特征影响。
- sensor position 不稳定时，算法估算的不确定性应提高。
- 该来源只支持信号质量解释，不支持患者级临床结论。

## 实现使用说明

- 用于 sensor_position、wearable PPG 和 signal_quality 检索。

## 全文候选与下载材料摘要

### Determinants of photoplethysmography signal quality at the wrist

- candidate_id: `charlton_2025_wrist_ppg_signal_quality_pdf`
- access_mode: `public_pdf`
- fulltext_status: local_pdf_available
- source_url: https://journals.plos.org/digitalhealth/article?id=10.1371/journal.pdig.0000585
- doi: 10.1371/journal.pdig.0000585
- pmid: 40577339
- access_recorded: 2026-05-16
- local_pdf: `charlton_2025_wrist_ppg_signal_quality.pdf`

#### 摘要入库要点

- 已从 PLOS Digital Health 官方 printable PDF 下载；本项目只提交中文摘要、citation 和访问记录，不提交全文 PDF。
- 该研究分析腕部 PPG 信号质量决定因素，支持传感器位置、佩戴/接触、姿势、活动状态和个体差异会影响光学波形质量。
- 报告可用该来源解释 wrist/sensor position 不稳定时估算不确定性更高，并建议重新放置传感器或用规范设备复核。
- 该来源只用于 signal_quality 和 research_background，不用于血压分类、诊断、治疗或调药。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `8e5771b4058f7399`

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。
- 研究或综述来源只用于背景和局限说明，不用于患者级临床建议。

