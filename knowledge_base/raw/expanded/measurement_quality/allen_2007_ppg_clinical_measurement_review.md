---
source_id: allen_2007_ppg_clinical_measurement_review
title: Photoplethysmography and its application in clinical physiological measurement
source_type: peer_reviewed_review
organization: Physiological Measurement
region: global
topic: measurement_quality
language: en
url: https://pubmed.ncbi.nlm.nih.gov/17322588/
doi: 10.1088/0967-3334/28/3/R01
pmid: '17322588'
year: '2007'
last_accessed: '2026-05-10'
evidence_class: review
source_quality_score: '19'
allowed_uses:
- signal_quality
- cuffless_ppg_limitations
- remeasurement
- research_background
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: 78af718e41fba721
safety_level: standard
---

## 来源摘要

Allen 2007 是 PPG 生理测量的基础综述，可用于解释 PPG 是光学、位点相关、易受采集条件影响的间接生理信号。

## 可用于报告的要点

- PPG 信号依赖光照、组织灌注和测量位置，不是直接血压测量。
- 运动、环境光、探头接触和传感器位置都可能改变波形质量。
- 低质量或不稳定波形应优先重新采集并用规范血压计复核。

## 实现使用说明

- 用于 PPG signal_quality 的背景说明；不用于患者级诊断或治疗建议。

## 全文候选与下载材料摘要

已检测到本地 PDF：allen_2007_ppg_clinical_measurement_review.pdf。请先把人工摘要写入 tracked fulltext_summaries 或同名 .summary.md 后再进入知识库，避免复制受版权保护全文。

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。
- 研究或综述来源只用于背景和局限说明，不用于患者级临床建议。

