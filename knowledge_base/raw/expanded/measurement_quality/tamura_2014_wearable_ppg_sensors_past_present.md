---
source_id: tamura_2014_wearable_ppg_sensors_past_present
title: Wearable Photoplethysmographic Sensors - Past and Present
source_type: review_pdf
organization: Electronics
region: global
topic: measurement_quality
language: en
url: https://www.mdpi.com/2079-9292/3/2/282
doi: 10.3390/electronics3020282
pmid: ''
year: '2014'
last_accessed: '2026-05-21'
evidence_class: review
source_quality_score: '21'
allowed_uses:
- signal_quality
- research_background
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: 04d1acf73632b930
safety_level: standard
---

## 来源摘要

该综述回顾可穿戴 PPG 传感器原理、传感器形态、佩戴位置和应用限制，可补充传感器位置与佩戴条件对信号的影响。

## 可用于报告的要点

- PPG 信号依赖光源、探测器、组织接触和测量部位。
- 可穿戴 PPG 适合趋势和背景解释，但不能直接转写为血压诊断能力。
- 报告可用该来源支持传感器位置和佩戴稳定性提示。

## 实现使用说明

- 用于 signal_quality 和 research_background。

## 全文候选与下载材料摘要

### Wearable Photoplethysmographic Sensors - Past and Present

- candidate_id: `tamura_2014_wearable_ppg_sensors_past_present_pdf`
- access_mode: `public_pdf`
- fulltext_status: local_pdf_available
- source_url: https://www.mdpi.com/2079-9292/3/2/282
- doi: 10.3390/electronics3020282
- pmid: n/a
- access_recorded: 2026-05-21
- local_pdf: `tamura_2014_wearable_ppg_sensors_past_present.pdf`

#### 摘要入库要点

- 已从 MDPI 官方开放 PDF 下载；本项目只提交中文摘要、citation 和访问记录，不提交全文 PDF。
- 该综述支持 PPG 信号依赖光源、探测器、组织接触、佩戴位置和测量部位。
- 报告可用该来源说明传感器位置和佩戴稳定性会影响信号质量；不用于临床诊断。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `fc32bab483269f88`

已检测到本地 PDF：tamura_2014_wearable_ppg_sensors_past_present.pdf。知识库只使用上述摘要和 citation，不复制全文。

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。
- 研究或综述来源只用于背景和局限说明，不用于患者级临床建议。

