---
source_id: shirbani_2020_ambient_light_skin_tone_vpg
title: Effect of Ambient Lighting and Skin Tone on Estimation of Heart Rate and Pulse
  Transit Time from Video Plethysmography
source_type: video_ppg_lighting_skin_tone_study
organization: IEEE EMBC
region: global
topic: measurement_quality
language: en
url: https://ieeexplore.ieee.org/document/9176731
doi: 10.1109/EMBC44109.2020.9176731
pmid: '33018549'
year: '2020'
last_accessed: '2026-05-10'
evidence_class: research_context
source_quality_score: '19'
allowed_uses:
- signal_quality
- research_background
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: 7849ef1e5523f0ef
safety_level: standard
---

## 来源摘要

该研究聚焦视频 PPG 中环境光和肤色对心率与脉搏传导时间估计的影响，可补充摄像头 PPG 场景的光照/肤色质量提示。

## 可用于报告的要点

- 环境光条件会影响视频/摄像头 PPG 的稳定性。
- 肤色和皮肤光学特性可能改变可获得的 PPG 信号质量。
- 报告可建议保持稳定、均匀、不过曝的光照，并避免把单次低质量结果作强解释。

## 实现使用说明

- 用于 ambient_light 和 skin_tone signal_quality；不用于个体化风险判断。

## 全文候选与下载材料摘要

### Effect of Ambient Lighting and Skin Tone on Estimation of Heart Rate and Pulse Transit Time from Video Plethysmography

- candidate_id: `shirbani_2020_ambient_light_skin_tone_vpg_fulltext`
- access_mode: `institution_or_browser`
- fulltext_status: summarized
- source_url: https://ieeexplore.ieee.org/document/9176731
- doi: 10.1109/EMBC44109.2020.9176731
- pmid: 33018549
- access_recorded: 2026-05-22
- local_pdf: not saved

#### 摘要入库要点

- 已归档用户提供的本地 PDF；本项目只提交中文摘要、citation 和访问记录，不提交全文 PDF。
- 该研究支持视频/摄像头 PPG 会受到环境光条件和肤色/皮肤光学特性的影响，心率和脉搏传导时间估计都需要谨慎解释。
- 报告可用该来源提示保持稳定、均匀、不过曝光照，并避免将单次低质量摄像头 PPG 结果作强风险解释。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `10639e731503a567`

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。
- 研究或综述来源只用于背景和局限说明，不用于患者级临床建议。

