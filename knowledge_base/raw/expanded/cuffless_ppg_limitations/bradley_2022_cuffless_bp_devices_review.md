---
source_id: bradley_2022_cuffless_bp_devices_review
title: Cuffless Blood Pressure Devices
source_type: review_pdf
organization: American Journal of Hypertension
region: global
topic: cuffless_ppg_limitations
language: en
url: https://pmc.ncbi.nlm.nih.gov/articles/PMC9088838/
doi: 10.1093/ajh/hpac017
pmid: '35136906'
year: '2022'
last_accessed: '2026-05-21'
evidence_class: review
source_quality_score: '23'
allowed_uses:
- cuffless_ppg_limitations
- device_advice
- research_background
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: 040103a1f84e5c7e
safety_level: standard
---

## 来源摘要

该综述介绍无袖带血压设备的技术路径、潜在价值、验证挑战和监管/临床落地障碍，可补强本系统对设备边界的解释。

## 可用于报告的要点

- 无袖带血压设备仍面临验证、校准、真实世界适用性和临床整合方面的挑战。
- 报告可提示用户用经过验证的袖带设备复核 PPG 估算，而不是把估算值当成诊断。
- 综述来源不能替代指南或设备验证标准。

## 实现使用说明

- 用于 cuffless_ppg_limitations、device_advice 和 research_background；不推荐商业型号。

## 全文候选与下载材料摘要

### Cuffless Blood Pressure Devices

- candidate_id: `bradley_2022_cuffless_bp_devices_review_pdf`
- access_mode: `public_pdf`
- fulltext_status: local_pdf_available
- source_url: https://pmc.ncbi.nlm.nih.gov/articles/PMC9088838/
- doi: 10.1093/ajh/hpac017
- pmid: 35136906
- access_recorded: 2026-05-21
- local_pdf: `bradley_2022_cuffless_bp_devices_review.pdf`

#### 摘要入库要点

- 已从 Oxford/PMC 合法公开入口归档 PDF；本项目只提交中文摘要、citation 和访问记录，不提交全文 PDF。
- 该综述支持无袖带血压设备仍面临验证、校准、真实世界适用性和临床整合挑战。
- 报告可用该来源提示使用经验证袖带血压计复核 PPG 估算；不得推荐商业型号或替代临床测量。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `0f3abadaca6b2635`

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。
- 研究或综述来源只用于背景和局限说明，不用于患者级临床建议。

