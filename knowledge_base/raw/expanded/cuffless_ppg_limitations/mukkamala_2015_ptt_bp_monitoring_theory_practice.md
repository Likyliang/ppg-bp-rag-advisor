---
source_id: mukkamala_2015_ptt_bp_monitoring_theory_practice
title: Toward Ubiquitous Blood Pressure Monitoring via Pulse Transit Time - Theory
  and Practice
source_type: review_pdf
organization: IEEE Transactions on Biomedical Engineering
region: global
topic: cuffless_ppg_limitations
language: en
url: https://pmc.ncbi.nlm.nih.gov/articles/PMC4515215/
doi: 10.1109/TBME.2015.2441951
pmid: '26057530'
year: '2015'
last_accessed: '2026-05-21'
evidence_class: review
source_quality_score: '21'
allowed_uses:
- cuffless_ppg_limitations
- signal_quality
- research_background
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: 6ca34e232e801991
safety_level: standard
---

## 来源摘要

该综述梳理脉搏传导时间 PTT 用于连续血压估算的理论基础、传感器组合、校准需求和真实世界限制，可用于解释无袖带/PPG 血压估算仍属于间接估算。

## 可用于报告的要点

- PTT 与血压之间的关系受个体血管特性、传感器布置、校准和生理状态影响。
- PPG/PTT 方法可用于研究背景和趋势解释，但不能替代经验证袖带血压测量。
- 报告可用该来源说明无袖带估算需要谨慎解读和规范血压计复核。

## 实现使用说明

- 用于 cuffless_ppg_limitations、signal_quality 和 research_background；不用于诊断、治疗或设备准确性背书。

## 全文候选与下载材料摘要

### Toward Ubiquitous Blood Pressure Monitoring via Pulse Transit Time - Theory and Practice

- candidate_id: `mukkamala_2015_ptt_bp_monitoring_theory_practice_pdf`
- access_mode: `public_pdf`
- fulltext_status: local_pdf_available
- source_url: https://pmc.ncbi.nlm.nih.gov/articles/PMC4515215/
- doi: 10.1109/TBME.2015.2441951
- pmid: 26057530
- access_recorded: 2026-05-21
- local_pdf: `mukkamala_2015_ptt_bp_monitoring_theory_practice.pdf`

#### 摘要入库要点

- 已从 PMC/Europe PMC 合法公开入口归档 PDF；本项目只提交中文摘要、citation 和访问记录，不提交全文 PDF。
- 该综述说明 PTT/PPG 血压估算受个体血管特性、传感器布置、校准和生理状态影响。
- 报告可用该来源解释 PPG/PTT 估算只能作为趋势参考或研究背景，不能替代经验证袖带血压测量。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `a78d2ea0ddb415e8`

已检测到本地 PDF：mukkamala_2015_ptt_bp_monitoring_theory_practice.pdf。知识库只使用上述摘要和 citation，不复制全文。

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。
- 研究或综述来源只用于背景和局限说明，不用于患者级临床建议。

