---
source_id: who_2021_hypertension_pharmacological_treatment_guideline
title: Guideline for the pharmacological treatment of hypertension in adults
source_type: pharmacological_treatment_guideline
organization: World Health Organization
region: global
topic: medication_safety
language: en
url: https://iris.who.int/bitstreams/f062769d-f075-4a00-87af-0a2106e0bd04/download
doi: ''
pmid: ''
year: '2021'
last_accessed: '2026-05-11'
evidence_class: guideline
source_quality_score: '23'
allowed_uses:
- medication_safety
- special_population
- home_bp_monitoring
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: 6f22bf92bb6cc652
safety_level: high
---

## 来源摘要

WHO 成人高血压药物治疗指南可作为用药安全和临床随访边界来源，支持系统拒绝自行开药、调药或停药建议。

## 可用于报告的要点

- 药物治疗应由临床专业人员根据规范测量和整体风险评估决定。
- PPG 估算偏高或偏低不能作为自行调整药物的依据。
- 报告可建议保留记录并咨询医生。

## 实现使用说明

- 用于 medication_safety；不得输出具体药名、剂量或治疗方案。

## 全文候选与下载材料摘要

### Guideline for the pharmacological treatment of hypertension in adults

- candidate_id: `who_2021_hypertension_pharmacological_treatment_pdf`
- access_mode: `public_pdf`
- fulltext_status: local_pdf_available
- source_url: https://iris.who.int/bitstreams/f062769d-f075-4a00-87af-0a2106e0bd04/download
- doi: n/a
- pmid: n/a
- access_recorded: 2026-05-11
- local_pdf: `who_2021_hypertension_pharmacological_treatment_guideline.pdf`

#### 摘要入库要点

- 已从公开 WHO 链接下载 PDF；本项目只提交中文摘要、citation 和访问记录，不提交全文 PDF。
- 该指南只用于用药安全边界：药物治疗和随访应由临床专业人员基于规范测量和整体风险评估决定。
- 报告不得根据 PPG 估算输出具体药名、剂量、加药、减药或停药建议。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `ef0ace01b02f62fb`

已检测到本地 PDF：who_2021_hypertension_pharmacological_treatment_guideline.pdf。知识库只使用上述摘要和 citation，不复制全文。

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。

