---
source_id: kdigo_2024_ckd_guideline
title: KDIGO 2024 Clinical Practice Guideline for the Evaluation and Management of
  Chronic Kidney Disease
source_type: guideline
organization: Kidney Disease - Improving Global Outcomes
region: global
topic: special_population
language: en
url: https://kdigo.org/guidelines/ckd-evaluation-and-management/kdigo-2024-ckd-guideline/
doi: ''
pmid: ''
year: '2024'
last_accessed: '2026-05-16'
evidence_class: guideline
source_quality_score: '24'
allowed_uses:
- special_population
- medication_safety
- home_bp_monitoring
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: aa55b91b2a39b2df
safety_level: high
---

## 来源摘要

KDIGO 2024 CKD 指南可强化慢性肾病用户的保守复核和医生沟通提醒。

## 可用于报告的要点

- CKD 用户出现血压估算偏高时应更保守地使用规范设备复核并咨询医生。
- 报告不得提供肾病相关用药或治疗目标。
- 用药用户不应基于 PPG 估算自行调整方案。

## 实现使用说明

- 用于 CKD special_population 和 medication_safety。

## 全文候选与下载材料摘要

### KDIGO 2024 CKD guideline PDF

- candidate_id: `kdigo_2024_ckd_guideline_pdf`
- access_mode: `public_pdf`
- fulltext_status: local_pdf_available
- source_url: https://kdigo.org/guidelines/ckd-evaluation-and-management/kdigo-2024-ckd-guideline/
- doi: n/a
- pmid: n/a
- access_recorded: 2026-05-16
- local_pdf: `kdigo_2024_ckd_guideline.pdf`

#### 摘要入库要点

- 已从 KDIGO 官方公开 PDF 下载；本项目只提交中文摘要、citation 和访问记录，不提交全文 PDF。
- CKD 用户的血压相关解释需要结合肾功能、用药、合并症和整体临床风险，不能由 PPG 估算单独判断。
- 该来源只用于支持“慢性肾病用户应更保守复核并主动与医生沟通”的特殊人群提醒。
- 报告不得输出 CKD 个体化血压目标、药物方案、加药、减药或停药建议。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `a474190226595a5b`

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。

