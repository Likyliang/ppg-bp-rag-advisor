---
source_id: acc_aha_2025_full_guideline_record
title: 2025 AHA ACC Guideline for the Prevention Detection Evaluation and Management
  of High Blood Pressure in Adults
source_type: guideline_record
organization: American Heart Association / American College of Cardiology
region: AHA
topic: bp_categories
language: en
url: https://www.ahajournals.org/doi/10.1161/CIR.0000000000001356
doi: 10.1161/CIR.0000000000001356
pmid: ''
year: '2025'
last_accessed: '2026-05-10'
evidence_class: guideline
source_quality_score: '25'
allowed_uses:
- bp_category_reference
- home_bp_monitoring
- remeasurement
- lifestyle
- special_population
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: 1936f6be07e496d1
safety_level: standard
---

## 来源摘要

AHA/ACC 2025 成人高血压指南正式全文用于新版指南追溯、规范测量边界和 PPG 免责声明。

## 可用于报告的要点

- 规范测量、重复读数、合适袖带和经验证设备是血压解释前提。
- 家庭血压记录可支持医生沟通。
- PPG 或无袖带估算值不能替代指南要求的测量。

## 实现使用说明

- 用于 guideline traceability、bp_category_reference、home_bp_monitoring 和 PPG 安全边界。

## 全文候选与下载材料摘要

### 2025 AHA ACC adult high blood pressure guideline full text

- candidate_id: `acc_aha_2025_guideline_fulltext`
- access_mode: `browser_required`
- fulltext_status: local_pdf_available
- source_url: https://professional.heart.org/en/guidelines-statements/2025-ahaaccaanpaapaabcaccpacpmagsamaaspcnmapcnasgim-guideline-for-thehyp0000000000000249
- doi: 10.1161/CIR.0000000000001356
- pmid: n/a
- access_recorded: 2026-05-10
- local_pdf: `acc_aha_2025_full_guideline_record.pdf`

#### 摘要入库要点

- 已通过浏览器获取 AHA Journals 正式 PDF；本项目只提交中文摘要、citation 和访问记录，不提交全文 PDF。
- 全文提供 2025 AHA/ACC 成人高血压指南的可追溯版本依据，强调血压解释应基于规范测量、重复读数、合适袖带和经过验证的设备。
- 指南明确提示应避免依赖智能手表等无袖带设备作为准确血压测量来源，直到其精密性和可靠性获得更充分证明；该点用于强化 PPG 结果免责声明。
- 180/120 mmHg 级别、妊娠、CKD、糖尿病、用药和特殊人群内容只用于安全提醒、复测和医生沟通；系统不得输出诊断、开药、停药或治疗调整。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `de1d7379950a15f9`

已检测到本地 PDF：acc_aha_2025_full_guideline_record.pdf。知识库只使用上述摘要和 citation，不复制全文。

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。

