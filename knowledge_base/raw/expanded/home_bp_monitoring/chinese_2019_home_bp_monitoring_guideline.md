---
source_id: chinese_2019_home_bp_monitoring_guideline
title: 2019中国家庭血压监测指南
source_type: home_bp_monitoring_guideline_pdf
organization: 中国高血压联盟《家庭血压监测指南》委员会
region: CN
topic: home_bp_monitoring
language: zh
url: https://zhgxyzz.xml-journal.net/cn/article/pdf/preview/10.16439/j.cnki.1673-7245.2019.08.005.pdf
doi: 10.3969/j.issn.1000-3614.2019.07.003
pmid: ''
year: '2019'
last_accessed: '2026-05-28'
evidence_class: guideline
source_quality_score: '22'
allowed_uses:
- home_bp_monitoring
- remeasurement
- device_advice
- bp_category_reference
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: 403599d13ae153ba
safety_level: standard
---

## 来源摘要

《2019中国家庭血压监测指南》围绕家庭血压计选择、测量频率、测量时间、记录方式和家庭血压正常值等问题给出规范化建议，是中国场景家庭复测和连续记录的重要补充来源。

## 可用于报告的要点

- 家庭血压监测是高血压管理的重要组成，强调规范设备和固定流程。
- 报告可引用该来源解释为什么单次 PPG 估算后需要上臂式血压计复核和连续记录。
- 家庭血压记录适合辅助医生或基层医疗人员理解趋势。

## 实现使用说明

- 用于中国用户 home_bp_monitoring、remeasurement 和 device_advice；不生成诊断、治疗或用药调整。

## 全文候选与下载材料摘要

### 2019中国家庭血压监测指南

- candidate_id: `chinese_2019_home_bp_monitoring_guideline_pdf`
- access_mode: `public_pdf`
- fulltext_status: local_pdf_available
- source_url: https://zhgxyzz.xml-journal.net/cn/article/doi/10.16439/j.cnki.1673-7245.2019.08.005
- doi: 10.3969/j.issn.1000-3614.2019.07.003
- pmid: n/a
- access_recorded: 2026-05-28
- local_pdf: `chinese_2019_home_bp_monitoring_guideline.pdf`

#### 摘要入库要点

- 该指南强调家庭血压监测在高血压管理中的重要性，并讨论血压计选择、校准、测量次数、测量天数和家庭血压正常值。
- 报告可用该来源解释为什么 PPG 估算偏高后需要上臂式血压计复核，并把日期、时间、血压和状态连续记录。
- 该来源支持家庭复测和医生沟通，不支持用小程序单次估算结果作诊断或治疗调整。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `2d6194730ae61a35`

已检测到本地 PDF：chinese_2019_home_bp_monitoring_guideline.pdf。知识库只使用上述摘要和 citation，不复制全文。

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。

