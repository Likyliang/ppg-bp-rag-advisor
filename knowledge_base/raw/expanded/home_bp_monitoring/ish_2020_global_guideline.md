---
source_id: ish_2020_global_guideline
title: 2020 International Society of Hypertension Global Hypertension Practice Guidelines
source_type: guideline
organization: International Society of Hypertension
region: global
topic: home_bp_monitoring
language: en
url: https://ish-world.com/global-hypertension-practice-guidelines/
doi: ''
pmid: ''
year: '2020'
last_accessed: '2026-05-10'
evidence_class: guideline
source_quality_score: '21'
allowed_uses:
- home_bp_monitoring
- remeasurement
- lifestyle
- special_population
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: beea7d3e04e21000
safety_level: standard
---

## 来源摘要

ISH 全球实践指南可作为资源受限和全球场景下的测量与生活方式背景。

## 可用于报告的要点

- 强调规范测量、重复读数和长期管理。
- 生活方式建议属于健康教育而非治疗替代。
- 特殊人群应更保守地咨询医生。

## 实现使用说明

- 用于全球背景，不优先于 CN/AHA。

## 全文候选与下载材料摘要

### ISH 2020 global hypertension practice guideline public PDF

- candidate_id: `ish_2020_public_pdf`
- access_mode: `public_pdf`
- fulltext_status: downloadable
- source_url: https://ish-world.com/global-hypertension-practice-guidelines/
- doi: 10.1161/HYPERTENSIONAHA.120.15026
- pmid: n/a
- access_recorded: 2026-05-10
- local_pdf: not saved

#### 摘要入库要点

- ISH 指南适合用于全球实践背景，强调规范测量、重复读数和长期风险沟通。
- 生活方式内容只能作为健康教育和医生沟通准备，不能承诺降压幅度或替代治疗。
- 对特殊人群或合并基础病用户，报告应保持保守，建议用规范设备复核并咨询医生。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `3817154533a24c3f`

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。

