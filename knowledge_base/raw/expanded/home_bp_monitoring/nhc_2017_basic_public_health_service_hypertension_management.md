---
source_id: nhc_2017_basic_public_health_service_hypertension_management
title: 国家基本公共卫生服务规范（第三版）高血压患者健康管理服务规范
source_type: public_health_service_standard_pdf
organization: 原国家卫生计生委
region: CN
topic: home_bp_monitoring
language: zh
url: https://www.nhc.gov.cn/ewebeditor/uploadfile/2017/04/20170417104506514.pdf
doi: ''
pmid: ''
year: '2017'
last_accessed: '2026-05-27'
evidence_class: guideline
source_quality_score: '23'
allowed_uses:
- home_bp_monitoring
- remeasurement
- special_population
- emergency_alert
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: 02403d691c902f1c
safety_level: high
---

## 来源摘要

《国家基本公共卫生服务规范（第三版）》中的高血压患者健康管理服务规范可支持中国基层随访、分类干预和转诊边界说明。

## 可用于报告的要点

- 基层高血压管理强调建档、随访、分类干预和必要时转诊。
- 报告可引用该来源支持记录、复测和与基层医疗服务沟通。
- 急症或明显异常时应触发安全提醒，不由小程序作临床判断。

## 实现使用说明

- 用于 home_bp_monitoring、remeasurement、special_population 和 emergency_alert；不生成诊断或治疗调整。

## 全文候选与下载材料摘要

### 国家基本公共卫生服务规范（第三版）高血压患者健康管理服务规范

- candidate_id: `nhc_2017_basic_public_health_service_hypertension_management_pdf`
- access_mode: `public_pdf`
- fulltext_status: local_pdf_available
- source_url: https://www.nhc.gov.cn/ewebeditor/uploadfile/2017/04/20170417104506514.pdf
- doi: n/a
- pmid: n/a
- access_recorded: 2026-05-27
- local_pdf: `nhc_2017_basic_public_health_service_hypertension_management.pdf`

#### 摘要入库要点

- 从国家卫健委公开 PDF 归档；本项目只提交高血压健康管理相关中文摘要、citation 和访问记录，不提交全文 PDF。
- 该规范支持基层高血压管理中的建档、随访、分类干预和必要时转诊。
- 报告可用该来源支持记录、复测和基层医疗沟通；不能替代基层医生诊疗或生成治疗调整。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `59b83fe566f96169`

已检测到本地 PDF：nhc_2017_basic_public_health_service_hypertension_management.pdf。知识库只使用上述摘要和 citation，不复制全文。

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。

