---
source_id: nhc_2025_primary_care_hypertension_standard_ws_t_872
title: 基层医疗卫生机构高血压防治管理标准 WS/T 872-2025
source_type: national_health_standard
organization: 国家卫生健康委员会
region: CN
topic: home_bp_monitoring
language: zh
url: https://www.nhc.gov.cn/fzs/c100048/202509/2f3f7cce449145f8b361e70b3ed4ae9a/files/WS%20T%20872%E2%80%942025-20250930105429913.pdf
doi: ''
pmid: ''
year: '2025'
last_accessed: '2026-05-11'
evidence_class: guideline
source_quality_score: '25'
allowed_uses:
- home_bp_monitoring
- remeasurement
- lifestyle
- special_population
review_status: included
derived_from: knowledge_base/sources/source_catalog.yaml
source_hash: c9742f486a799bfa
safety_level: standard
---

## 来源摘要

WS/T 872-2025 为基层医疗卫生机构高血压防治管理提供国家行业标准，可强化中文场景下的筛查、随访、健康教育和规范管理边界。

## 可用于报告的要点

- 中文报告可引用该来源支持基层健康管理、复测记录和医生沟通。
- 治疗和随访流程不得改写成小程序自动诊断或治疗方案。
- PPG 估算结果仍需规范血压测量复核。

## 实现使用说明

- 用于 CN 场景 home_bp_monitoring、remeasurement 和 special_population。

## 全文候选与下载材料摘要

### 基层医疗卫生机构高血压防治管理标准 WS/T 872-2025

- candidate_id: `nhc_2025_primary_care_hypertension_standard_pdf`
- access_mode: `public_pdf`
- fulltext_status: local_pdf_available
- source_url: https://www.nhc.gov.cn/fzs/c100048/202509/2f3f7cce449145f8b361e70b3ed4ae9a/files/WS%20T%20872%E2%80%942025-20250930105429913.pdf
- doi: n/a
- pmid: n/a
- access_recorded: 2026-05-11
- local_pdf: `nhc_2025_primary_care_hypertension_standard_ws_t_872.pdf`

#### 摘要入库要点

- 已从公开国家卫健委链接下载 PDF；本项目只提交中文摘要、citation 和访问记录，不提交全文 PDF。
- 该标准可用于中文场景下基层血压管理、随访记录、健康教育和规范复核建议。
- 治疗流程和管理标准不得被改写成小程序自动诊断或治疗方案；PPG 估算仍只作为趋势参考。

#### 使用边界

- 只把上述摘要用于 PPG 估算解释、复测建议、设备局限、生活方式教育或安全提醒。
- 不复制全文、不引用未审校段落、不生成诊断、治疗、开药、停药或替代规范血压测量的结论。
- candidate_hash: `845abfe8c389e3ca`

已检测到本地 PDF：nhc_2025_primary_care_hypertension_standard_ws_t_872.pdf。知识库只使用上述摘要和 citation，不复制全文。

## 安全边界

- 该来源只用于 PPG 估算结果解释、复测建议、生活方式建议、就医提醒或安全边界。
- 不得把该来源改写成本系统可以诊断、治疗、开药、停药或替代规范血压测量的依据。

