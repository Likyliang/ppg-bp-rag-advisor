...
| 测试数 | 自动化测试数量，覆盖 schema、规则引擎、安全边界、检索、API 和报告生成 | `224 passed` 表示当前代码、知识库处理流程、API 实验脚本和并发限制逻辑通过了自动化回归测试 |
...
| 报告 P95 | 95% 报告生成请求的耗时上限 | `0.212s` 表示当前 template/rule-based 报告生成仍然很快，适合 Demo 展示 |
...
在严格质量门禁之外，本周还增加了一轮轻量 API 实验，用现有 FastAPI 路由直接跑小程序结构化病例输入，不新增接口、不改变主评估口径。实验脚本 `scripts/run_api_experiments.py` 调用 `/api/v1/reports/preview-rules`、`/api/v1/reports/generate` 和 `/api/v1/kb/search`，并使用同一个最大并发 5 限制。

实验覆盖 50 个 report fixtures 和 6 个 KB search 探针，结果为：报告生成成功率 1.0，Safety pass rate 1.0，输入上下文保留率 1.0，证据覆盖率 1.0，建议级证据绑定率 1.0，敏感场景高可信证据率 1.0，急症识别一致性 1.0，API P95 latency 0.1691s，KB search expected-use hit rate 1.0。输出文件位于 `outputs/experiments/api_experiment_2026-05-21.json`、`.csv` 和 `.md`。

## 9. 可用于周报的一句话总结

本周完成了高血压 PPG 估算解释 RAG-Agent 的知识库治理和全文文献摘要化扩展：系统明确以小程序结构化 PPG 估算输出为输入边界，累计归档维护 62 份完整 PDF 文献/资料，筛选后形成 114 个纳入来源、524 个 chunks，并建立了从文献下载、PDF 归档、source catalog、中文摘要、metadata chunk、检索评估、报告评估到 Obsidian 文献管理的闭环；在新增无袖带 BP、PTT 校准、PPG 信号质量、运动伪差和肤色公平性证据后，严格质量门禁仍全部通过，224 个测试通过，检索 precision@5 为 1.0，unsafe-source leakage 为 0，建议级证据绑定率为 1.0；额外 API 链路实验显示 50 个病例均可通过现有 FastAPI 路由生成安全、带证据的报告，实验评估支持并行但最大并发限制为 5。
