...
## 个性化触发因素
- 读数触发：估算值 146.0/92.0 mmHg 对应 `CN_stage_2_reference_range`，所以进入“处于明显偏高范围参考值”解释。
- 质量触发：本次信号质量可用于生成保守的趋势解释。
...
## 为什么给出这些建议
- 因为估算值进入偏高或严重偏高参考范围，报告优先建议复测、连续记录和医生沟通。
- 生活方式内容只作为通用健康教育，用于减盐、运动、体重和睡眠等长期管理，不承诺个体降压效果。
