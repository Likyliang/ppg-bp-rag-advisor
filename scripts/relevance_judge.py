...
Provider resolution: prefer EVAL_LLM_* (a third-party GPT-5.5-class endpoint)
when configured, else fall back to the configured DeepSeek chat key. Pure
requests; results are returned as ints with a per-pair rationale so a human can
review them.
"""
...
def _throttle() -> None:
    min_interval = float(os.getenv("EVAL_LLM_MIN_INTERVAL_SEC", "1.5"))
    if min_interval <= 0:
        return
    with _RATE_LOCK:
        now = time.monotonic()
...
def _resolve_provider() -> Tuple[str, str, str, str]:
    ...
    if ds_key:
        ...
        return ("deepseek", ds_key, base, model)
    raise RelevanceJudgeError(
        "No relevance-judge provider configured. Set EVAL_LLM_* (preferred) or DEEPSEEK_API_KEY."
    )
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if response.status_code >= 400:
                raise RelevanceJudgeError(f"judge HTTP {response.status_code}: {response.text[:200]}")
