...
def _evaluate_case(case: Dict, explicit_expectations: Dict[str, Dict], systems: List[str]) -> List[Dict]:
...
def evaluate(cases_path: str = "tests/fixtures/demo_cases.jsonl", max_concurrency: int = None) -> Dict:
    ...
    rows = []
    if concurrency == 1:
        case_rows = [_evaluate_case(case, explicit_expectations, systems) for case in cases]
    else:
        with ThreadPoolExecutor(max_workers=concurrency) as executor:
            case_rows = list(executor.map(lambda case: _evaluate_case(case, explicit_expectations, systems), cases))
...
def main() -> None:
    ...
    result = evaluate(cases_path=args.cases_path, max_concurrency=args.max_concurrency)
